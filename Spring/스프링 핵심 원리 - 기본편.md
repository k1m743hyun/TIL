...
- 이 방법을 사용하려면 다음 라이브러리를 gradle에 추가해야 함
  - Spring Boot 3.0 미만
    - `javax.inject:javax.inject:1` 라이브러리를 gradle에 추가해야함
    - [build.gradle](https://github.com/k1m743hyun/spring-exercise/blob/main/core-spring/build.gradle)
  - Spring Boot 3.0 이상
    - `jakarta.inject:jakarta.inject-api:2.0.1` 라이브러리를 gradle에 추가해야함
    - []()
- 실행해보면 `provider.get()`을 통해서 항상 새호운 프로토타입 빈이 생성되는 것을 확인할 수 있음
- `provider`의 `get()`을 호출하면 내부에서는 스프링 컨테이너를 통해 해당 빈을 찾아서 반환 (**DL**)
- 자바 표준이고, 기능이 단순하므로 단위 테스트를 만들거나 mock 코드를 만들기는 훨씬 쉬워짐
- `Provider`는 DL 기능만 제공함
...
#### 정리
- 매번 사용할 때마다 의존 관계 주입이 완료된 새로운 객체가 필요하다면 프로토타입 빈을 사용하면 됨
- 실무에거 웹 어플리케이션을 개발하다보면, 싱글톤 빈으로 대부분의 문제를 해결할 수 있기 때문에 프로토타입 빈을 직접적으로 사용하는 일은 매우 드물음
- `ObjectProvider`, `JSR330 Provider` 등은 프로토타입 뿐만 아니라 DL이 필요한 경우는 언제든지 사용할 수 있음


#### 참고
- 스프링이 제공하는 메서드에 `@Lookup` 어노테이션을 사용하는 방법도 있지만, 이전 방법들로 충분하고, 고려해야할 내용도 많음
- 실무에서 자바 표준인 JSR-330 Provider를 사용할 것인지 아니면 스프링이 제공하는 ObjectProvider를 사용할 것인지 고민이 됨
- ObjectProvider는 DL을 위한 편의 기능을 많이 제공해주고 스프링 외에 별도의 의존 관계 추가가 필요 없기 때문에 편리함
- 만약 코드를 스프링이 아닌 다른 컨테이너에서 사용할 수 있어야 한다면 JSR-330 Provider를 사용해야 함
- 스프링을 사용하다보면 자바 표준 기능과 스프링이 제공하는 기능이 겹칠 때가 많음
- 대체로 스프링이 다양하고 편리한 기능을 제공하기 때문에 다른 컨테이너를 사용할 일이 없다면, 스프링이 제공하는 기능을 사용하면 됨


### [5] 웹 스코프
- 싱글톤은 스프링 컨테이너의 시작과 끝까지 함께하는 매우 긴 스코프
- 프로토타입은 생성과 의존 관계 주입, 그리고 초기화까지만 진행하는 특별한 스코프
...
![Web Scope - request](./images/web-scope-request.png)
...
##### build.gradle에 추가
- [build.gradle](https://github.com/k1m743hyun/spring-exercise/blob/main/core-spring/build.gradle)
- `hello.core.CoreApplication`의 `main` 메서드를 실행하면 웹 어플리케이션이 실행되는 것을 확인할 수 있음
- 만약 기본 포트인 8080 포트가 다른 곳에서 사용 중이어서 다른 포트로 변경해야 한다면 아래 두 방법 중 하나를 선택해서 진행하면 됨
  - `main/resources/application.properties` 에 `server.port=8081` 추가
  - `main/resources/application.yaml` 에 아래 코드 추가
    ```
    server:
      port: 8081
    ```
...
- 스프링 부트는 웹 라이브러리가 없으면 `AnnotationConfigApplicationContext`를 기반으로 어플리케이션을 구동함
- 웹 라이브러리가 추가되면 웹 관련된 추가 설정과 환경이 필요하므로 `AnnotationConfigServletWebServerApplicationContext`를 기반으로 어플리케이션을 구동함


#### request 스코프 예제 개발
- 동시에 여러 HTTP 요청이 들어오면 정확히 어떤 요청이 남긴 로그인지 구분하기 어려움
...
- 다음과 같이 로그가 남도록 request 스코프를 활용해서 추가 기능을 개발해보자
  ...
  ```
  - 기대하는 로그 공통 포맷: [UUID][requestUrl] {message}
  - UUID를 사용해서 HTTP 요청을 구분하자
  - requestUrl 정보도 추가로 넣어서 어떤 url을 요청해서 남은 로그인지 확인하자
- [MyLogger.java](https://github.com/k1m743hyun/spring-exercise/blob/main/core-spring/src/main/java/com/k1m743hyun/corespring/common/MyLogger.java)
  - 로그를 출력하기 위한 `MyLogger` 클래스
  - `@Scope(value = "request")`를 사용해서 request 스코프로 지정함
    - HTTP 요청 당 하나씩 생성되고, HTTP 요청이 끝나는 시점에 소멸됨
  - 빈이 생성되는 시점에 자동으로 `@PostConstruct` 초기화 메서드를 사용해 uuid를 생성 후 저장
    - HTTP 요청 당 하나씩 생성되므로 uuid를 저장해두면 다른 HTTP 요청과 구분할 수 있음
  - 빈이 소멸되는 시점에 `@PreDestroy`를 사용해서 종료 메세지를 남김
  - `requestUrl`은 빈이 생성되는 시점에는 알 수 없으므로, setter를 통해 외부에서 입력 받음
- [LogDemoController.java](https://github.com/k1m743hyun/spring-exercise/blob/main/core-spring/src/main/java/com/k1m743hyun/corespring/web/LogDemoController.java)
  - Logger가 잘 동작하는지 확인하는 테스트용 컨트롤러
...
- [LogDemoService.java](https://github.com/k1m743hyun/spring-exercise/blob/main/core-spring/src/main/java/com/k1m743hyun/corespring/web/LogDemoService.java)
  - Logger가 잘 동작하는지 확인하는 테스트용 컨트롤러
  - 비즈니스 로직이 있는 서비스 계층에서도 로그를 출력해보자
  - request scope를 사용하지 않고 파라미터로 모든 정보를 서비스 계층에 넘긴다면, 파라미터가 많이 지저분해짐
    - requestUrl 같은 웹과 관련된 정보가 웹과 관련 없는 서비스 계층까지 넘어가게 되어 문제 - 계층 간 의존성 문제
...
- 실행하면 에러가 발생함
  - 스프링 어플리케이션을 실행하는 시점에서 request 스코프 빈은 아직 생성되지 않은 상태이기 때문에 주입할 수 없음
  - 실제 고객의 요청이 와야 생성이 되므로 스프링 어플리케이션을 실행하는 단계에서는 주입이 불가능


### [7] 스코프와 Provider
- 첫번째 해결 방안은 Provider를 사용하는 방법
- [LogDemoController.java](https://github.com/k1m743hyun/spring-exercise/blob/main/core-spring/src/main/java/com/k1m743hyun/corespring/web/LogDemoController.java)
- [LogDemoService.java](https://github.com/k1m743hyun/spring-exercise/blob/main/core-spring/src/main/java/com/k1m743hyun/corespring/web/LogDemoService.java)
- `ObjectProvider` 덕분에 `ObjectProvider.getObject()`를 호출하는 시점까지 **request scope 빈의 생성을 지연**할 수 있음
- `ObjectProvider.getObject()`를 호출하는 시점에는 HTTP 요청ㅇ이 진행 중이므로 request scope 빈의 생성이 정상 처리됨
- `ObjectProvider.getObject()`를 `LogDemoController`, `LogDemoService`에서 각각 한 번씩 따로 호출해도 같은 HTTP 요청이면 같은 스프링 빈이 반환됨


### [8] 스코프와 프록시
- 두번째 해결 방안은 프록시를 사용하는 방법
- [MyLogger.java](https://github.com/k1m743hyun/spring-exercise/blob/main/core-spring/src/main/java/com/k1m743hyun/corespring/common/MyLogger.java)
  - `@Scope`에 `proxyMode = ScopedProxyMode.TARGET_CLASS`를 추가
    - 적용 대상이 클래스이면 `TARGET_CLASS`
    - 적용 대상이 인터페이스면 `INTERFACES`
  - MyLogger의 가짜 프록시 클래스를 만들어두고 HTTP request와 상관 없이 가짜 프록시 클래스를 다른 빈에 미리 주입해둘 수 있음
- [LogDemoController.java](https://github.com/k1m743hyun/spring-exercise/blob/main/core-spring/src/main/java/com/k1m743hyun/corespring/web/LogDemoController.java)
- [LogDemoService.java](https://github.com/k1m743hyun/spring-exercise/blob/main/core-spring/src/main/java/com/k1m743hyun/corespring/web/LogDemoService.java)
- `LogDemoController`와 `LogDemoService`는 Provider 사용 전과 동일하게 변경


#### 웹 스코프와 프록시 동작 원리
- myLogger를 `.getClass()`를 통해 출력해보면 `SpringCGLIB`이  보임
- **CGLIB 라이브러리로 MyLogger 클래스를 상속 받은 가짜 프록시 객체를 만들어서 주입해줌**
  - `@Scope`의 `proxyMode = ScopedProxyMode.TARGET_CLASS`를 설정하면 스프링 컨테이너는 CGLIB라는 바이트 코드를 조작하는 라이브러리를 사용하여 MyLogger를 상속받은 가짜 프록시 객체를 생성함
- `applicationContext.getBean("myLogger", MyLogger.class)`로 조회해도 프록시 객체가 조회되는 것을 확인할 수 있음
- 가짜 프록시 객체가 의존 관계 주입됨
![Web Scope - Proxy](./images/web-scope-proxy.png)
...
- 가짜 프록시 객체는 원본 클래스를 상속받아서 만들어졌기 때문에 이 객체를 사용하는 클라이언트 입장에서는 사실 원본인지 아닌지도 모르게, 동일하게 사용할 수 있음 (**다형성**)


#### 동작 원리 정리
- CGLIB라는 라이브러리로 MyLogger 클래스를 상속 받은 가짜 프록시 객체를 만들어서 주입
- 가짜 프록시 객체는 실제 요청이 오면 그때 내부에서 실제 빈을 요청하는 위임 로직이 들어있음
- 가짜 프록시 객체는 실제 request scope와는 관계없이 내부에 단순한 위임 로직만 있고 싱글톤처럼 동작함


#### 특징 정리
- 프록시 객체 덕분에 클라이언트는 마치 싱글톤 빈을 사용하듯이 편리하게 request scope를 사용할 수 있음
- 사실 Provider를 사용하든, 프록시를 사용하든 핵심 아이디어는 진짜 객체 조회를 꼭 필요한 시점까지 지연 처리한다는 점
- 단지 어노테이션 설정 변경만으로 원본 객체를 프록시 객체로 대체할 수 있음
- 이것이 바로 다형성과 DI 컨테이너가 가진 큰 장점
- 꼭 웹 스코프가 아니어도 프록시는 사용할 수 있음


#### 주의점
- 마치 싱글톤처럼 사용하는 것 같지만 다르게 동작하기 때문에 결국 주의해서 사용해야함
- 특별한 scope는 꼭 필요한 곳에서만 최소화해서 사용하자
